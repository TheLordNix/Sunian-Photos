...
class ImageCreateResp(BaseModel):
    ...
    public_id: Optional[str] = Field(None, description="Cloudinary public_id")
    responsive: Optional[List[str]] = Field(None, description="Responsive/thumbnail URLs")
...
class ImageEdit(BaseModel):
    ...
    privacy: Optional[str] = Field(None, description="Privacy setting (e.g., public, private, unlisted)")
...
class SearchQuery(BaseModel):
    q: Optional[str] = Field(None, description="Keyword to search in title, caption, filename, or tags")
    album_id: Optional[str] = Field(None, description="Filter by album ID")
    from_date: Optional[datetime.date] = Field(None, description="Filter images uploaded on or after this date (YYYY-MM-DD)")
    to_date: Optional[datetime.date] = Field(None, description="Filter images uploaded on or before this date (YYYY-MM-DD)")
    camera: Optional[str] = Field(None, description="Filter by camera model (from EXIF metadata)")
    license: Optional[str] = Field(None, description="Filter by license type (e.g., CC-BY, All Rights Reserved)")
    limit: Optional[int] = Field(50, ge=1, le=200, description="Maximum number of results to return (default 50)")
